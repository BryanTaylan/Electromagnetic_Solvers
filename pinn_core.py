...
EPOCHS = 5000         # Adam epochs. I chose 20k epochs because PINNs converge slowly, unlike supervised tasks, the loss here comes from physics residuals, which are noisier and harder to optimize. 
...
def train( weights, biases, activations, omegas, omega = OMEGA, epochs = EPOCHS, lr = LR, loss_threshold = EARLY_STOP_THR, lambda_bc = LAMBDA_BC, circle = None, n_interior = N_INTERIOR, n_boundary = N_BOUNDARY, use_lbfgs = USE_LBFGS, source_center = (0.0, 0.0)):
    ...
    for ep in range( epochs ):
        ...
        res_i = - lap_i - ( eps_i * ( omega**2 ) ) * Ei_i + ( omega * J ) # J instance here
