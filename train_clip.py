...
def train_clip(clip_model, classifier, train_loader, val_loader):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(classifier.state_dict(), "best_clip.pt")
...
def evaluate(clip_model, classifier, test_loader, checkpoint):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x, y = x.to(device), y.to(device)
            features = clip_model.encode_image(x).float()
            logits = classifier(features)
            preds = logits.argmax(dim=1)
...
def main():
    ...
    train_dataset = FieldDataset224(train_df)
    val_dataset   = FieldDataset224(val_df)
    test_dataset  = FieldDataset224(test_df)

    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)
    val_loader   = DataLoader(val_dataset,   batch_size=BATCH_SIZE, shuffle=False)
    test_loader  = DataLoader(test_dataset,  batch_size=BATCH_SIZE, shuffle=False)

    history = train_clip(clip_model, classifier, train_loader, val_loader)

    accuracy, cm, per_class_acc, inf_time = evaluate(clip_model, classifier, test_loader, "best_clip.pt")

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].plot(history["train_loss"], label="Train Loss")
    axes[0].plot(history["val_loss"],   label="Val Loss")
    axes[0].set_title("Loss — OpenCLIP")
    axes[0].legend()
    axes[1].plot(history["train_acc"], label="Train Acc")
    axes[1].plot(history["val_acc"],   label="Val Acc")
    axes[1].set_title("Accuracy — OpenCLIP")
    axes[1].legend()
    plt.tight_layout()
    plt.savefig("clip_curves.png", dpi=150)
    plt.close()
    print("Saved clip_curves.png")
